<x-portlets.base portlet-class="col-md-12" footer-class="m-portlet__foot--fit" :form=true form-action="{{ $data['form-action'] }}" form-method="{{ $data['form-method'] }}" :model="$data['setting']">
    ...
    <x-slot name="content">
        ...
        <div class="form-group m-form__group row">
            ...
            <x-inputs.text form-class="col-lg-6" :errors="$errors" label="Facebook Link" labelfor="facebook" name="facebook" type="text" value="{{  $data['setting']->facebook ?? null }}"></x-inputs.text>
        </div>

        <div class="form-group m-form__group row">
            <x-inputs.text form-class="col-lg-6" :errors="$errors" label="Instagram Link" labelfor="instagram" name="instagram" type="text" value="{{  $data['setting']->instagram ?? null }}"></x-inputs.text>
            <x-inputs.text form-class="col-lg-6" :errors="$errors" label="Twitter Link" labelfor="twitter" name="twitter" type="text" value="{{  $data['setting']->twitter ?? null }}"></x-inputs.text>
        </div>

        <div class="form-group m-form__group row">
            <x-inputs.image form-class="col-lg-6" :errors="$errors" label="Logo" labelfor="logo" name="logo" type="file" value="{{  $data['setting']->logo ?? null }}"></x-inputs.image>
        </div>
