<x-portlets.base portlet-class="col-md-12" footer-class="m-portlet__foot--fit" :form=true form-action="{{ $data['form-action'] }}" form-method="{{ $data['form-method'] }}" :model="$data['category']">
    ...
    <x-slot name="content">
        ...
        <div class="form-group m-form__group row">
            <x-inputs.text form-class="col-lg-12" :errors="$errors" label="Name" labelfor="name" name="name" type="text" value="{{  old('name') ?? ($data['category']->name ?? old('name') ?? null) }}"></x-inputs.text>
        </div>
